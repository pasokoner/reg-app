...
<main class="flex h-screen max-h-screen w-full flex-col">
	<div class="flex h-16 items-center justify-center bg-blue text-xl font-semibold">SCANNER</div>
	<!-- svelte-ignore a11y-media-has-caption -->
	<video bind:this={videoFeed} class="flex-1" />

	<form method="POST" use:enhance class="max-w-[600px] mx-auto space-y-4">
		...
		{#if $submitting}
			<div class="flex h-16 items-center justify-center gap-x-2 bg-orange-dark text-xl">
				<LoaderIcon class="animate-spin" /> Please wait...
			</div>
		{:else if successMessage}
			<div class={cn('flex h-16 items-center justify-center bg-green-dark text-xl')}>
				{successMessage}
			</div>
		{:else if $errors._errors && $errors._errors.length > 0}
			{#each $errors._errors as err}
				<div class={cn('flex h-16 items-center justify-center bg-green-dark text-xl bg-red-800')}>
					{err}
				</div>
			{/each}
		{/if}
